...
\begin{compactitem}
	\item Work for Computer Engineering and one other engineering major, Web design was chosen
	\item Select any AP and IB tests taken and receive scores, and transfer credits and generate first year schedule
	\item If two quarters in a row have a CORE slot, and C\&I sequence in those slots
...
The following use case scenario provides a brief but in-depth explanation of the web application functions and how a user would interact with them.  \Cref{Use} is the use case diagram, which illustrates how the web application work.  The student (user) has the ability to choose his or her major, select any previously taken tests and add the respective scores, select courses that are covered by any previous credit, and view and print the proposed schedule. Below is an in-depth description of the use case scenarios shown in \Cref{Use}.\\
Actor: Student\\
Goal: Select major\\
...
SCUCourses.fish will be an online application and users will be able to access our application through any of the supported web browers. The architectural diagram for the application is shown in \Cref{Arch}. The user will log onto the internet using a web browser and navigate to SCUCourses.fish, and it will then pull all our information from the servers once. The user will then be able to use our application by interacting with the browser.
...
Knowledge of web languages & Progress on coding is greatly slowed & 0.6 & 2 & 1.2 & Model our code after some appropriate pre-existing examples of the language & Redistribute coding duties that may be difficult for one or more group members\\\hline
...
Now you are ready to create your schedule. You need to select between the Computer Engineering (COEN) major or the Web Design and Engineering major to begin to generate a schedule. There is a standard schedule that has been created at the bottom of the page. From this point on you have the ability to view the schedule, which will update automatically as you continue to answer the questions that follow. A question will appear under the previous question, and it will ask you how certain you are about the major you have selected. You can select ``Fairly Confident'' or ``I’m secretly terrified I should have picked another''. Next another question will appear that asks you if you have any experience with C programming. You can answer ``Oh, yeah'' if you do have experience or ``I know that I know nothing'' if you do not.

Now you are presented with the option to enter AP or IB credit. If you took any AP and/or IB exams select the ``AP'' and/or ``IB'' buttons, respectively. A series of buttons for each test accepted by SCU will populate below. Click all the applicable test buttons. A list of your selected tests with clickable scores to their right will have appeared. Click the associated scores for each of these tests. If you accidentally select a score for a test you did not take, simply click the test button above to remove the score’s effect from the schedule. You can even remove the entire AP or IB sections by clicking on the AP or IB button.

Below this a question will ask you if you passed the calculus readiness exam. You can select ``Yes, with flying colors'' if you did or ``Just put me in pre-calc'' if you did not. Below this you are asked if you have any transfer credit? Select ``But of course'' if you have transfer credit or ``Sadly, I don’t'' if you do not. If you do have transfer credit you will be prompted to select the field(s) in which you do have credit. You can choose ``Science,'' ``COEN,'' and/or ``Math.'' Once clicked, checkboxes with possible transfer credit courses will populate below. Some of these checkboxes may override others as credit in a course further along in a series of courses implies credit in the previous ones. Clicking a section again will remove the selected transfer credits from that section and hide the checkboxes.
