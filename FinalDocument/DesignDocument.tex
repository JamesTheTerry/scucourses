...
The following are the requirements to fit with the needs expressed to our team by the customer. They are split up between functional and non-functional, functional deals with the hard requirements of the system, while non-functional deals how the system runs.
...
\begin{compactitem}
	\item Select desired Major
	\item Select any AP and IB tests taken and give scores
	\item Give the score of their calculus readiness exam if applicable
	\item Select courses believed to be covered by previously earned credits
	\item View a proposed schedule
...
\begin{compactitem}
	...
	\item be easily shared with incoming freshmen
\end{compactitem}
...
Actor: Student\\
Goal: Select major and generate schedule\\
Pre-condition: Actor is a new student at Santa Clara University\\
Post-condition: Actor has successfully chosen a major and viewed the default schedule\\
Scenario:
\begin{compactitem}
...
Exceptions:
\begin{compactitem}
	\item Design Center server crashes
	\item Actor becomes disconnected from the internet
\end{compactitem}
Actor: Student\\
Goal: Input test scores and generate schedule\\
...
Pre-condition: Actor is a new student at Santa Clara University and chosen a major\\
Post-condition: Actor has successfully removed covered classes and viewed an optimized schedule\\
Scenario:
\begin{compactitem}
...
Pre-condition: Actor is a new student at Santa Clara University and chosen a major\\
Post-condition: Actor has successfully viewed an optimized schedule and printed said schedule\\
Scenario:
\begin{compactitem}
	\item Click print
\end{compactitem}
...
To best fulfill the requirements detailed in Section 2 and meet the design shown in Section 3, we will use the following technology:
\begin{compactitem}
...
Time runs out & Not all desired functions of the project completed by the turn-in date & .3 & 9 & 2.7 & Cut some less-essential features & Perform extensive time management throughout the course of production\\\hline

Bugs and Errors & We'll have to go back into the code, find out what's causing the problems, and bugfix them & .7 & 2 & 1.4 & Do ample testing every step of the way to find bugs sooner & Make use of debugging softwear\\\hline

Version control & Entire pages of code must be rewritten, causing tremendous setbacks towards the project's completion & .05 & 5 & .25 & Download permanent local copies of all documents periodically & Divide the code up into multiple documents, to minimize individual risk\\\hline

Knowledge of web languages & Progress on coding is greatly slowed & .1 & 2 & .2 & Model our code after some appropriate pre-existing examples of said language before hand & Redistribute coding duties that may be difficult for one or more group members\\\hline

Personnel & Work for that person's assigned section of the project falls behind & .02 & 8 & .16 & Get a significant amount of the work done ahead of time to compensate for possible losses in personnel & Keep a list of understudies so we can re-assign sections to currently active members \\\hline
